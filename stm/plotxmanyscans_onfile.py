...
f = open("[Com COM18] (2021-01-17_160517).log", "r")
...
for line in f:
    ...
    if line != "\n" and line != "STARTING X-ax SCAN\n":
        ...
        if "SCAN DIR 1 COMPLETE" in line: # just completed backwards scan, about to print forwards scan
            ...
            current_list_buffer.append(float(current)*1e-12)
...
counter = 0
for xpos_list,zpos_list,current_list, index in zip(xpos_lists_forward, zpos_lists_forward, current_lists_forward, enumerate(xpos_lists_forward)):
    counter += 1
    #print(index[0])
